...
class Data_small:
    def __init__(self,
        ...
        """
        # read with pandas
        parameter_df = pd.read_csv('小算例\\parameter.csv')
        demand_df = pd.read_csv('小算例\\demand_'+str(node_num)+'.csv')
        node_df = pd.read_csv('小算例\\node_'+str(node_num)+'.csv')
        time_matrix_df = pd.read_csv('小算例\\time_matrix_'+str(node_num)+'.csv')
